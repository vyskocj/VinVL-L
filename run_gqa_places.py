...
def _load_img_features(args):
    ...
    if args.img_feature_type == 'faster_r-cnn':
        ...
        feat_file_name = 'gqa_img_frcnn_feats.pt'
    with open(os.path.join(args.data_dir, feat_file_name), 'rb') as f:
        buffer = io.BytesIO(f.read())
    img_features = torch.load(buffer)
    t_end = time.time()
...
class GQADataset(Dataset):
    ...
    def __init__(self, args, name, img_features, tokenizer, label_pos_feats=None):
        ...
        if args.places_io_json != "":
            self.places_io_pred = json.load(open(args.places_io_json, "r"))
        else:
            self.places_io_pred = None
        if args.places_c_json != "":
            self.places_cat_pred = json.load(open(args.places_c_json, "r"))
        else:
            self.places_cat_pred = None
        if args.places_feats_hdf5 != "":
            import h5py
            self.places_feat = h5py.File(args.places_feats_hdf5, "r")
        else:
            self.places_feat = None
    ...
    def tensorize(self, cls_token_at_end=False, pad_on_left=False,
        ...
        # logger.info("tensorize:")  # debug: not used during training
        # debug:
        debug_size = 500
        features = []
        ...
        for (ex_index, example) in enumerate(self.examples[0: ]):
            ...
            tokens_b = None
            if example.text_b:
                # add places to object tags -> "<place category> <Indoor/Outdoor> <detected objects>"
                places_obj_tags = ""
                if self.places_cat_pred is not None:
                    places_obj_tags += self.places_cat_pred[example.img_key] + " "
                if self.places_io_pred is not None:
                    places_obj_tags += self.places_io_pred[example.img_key] + " "

                # transfer object tags to tokens
                tokens_b = self.tokenizer.tokenize(places_obj_tags + example.text_b)
                # logger.info(f"text_b full: {places_obj_tags + example.text_b}")  # debug
                # logger.info(f"tokens_b full: {tokens_b}")                        # debug
            ...
            # logger.info(f"img_feat.shape before: {img_feat.shape}")  # debug
            if self.places_feat is not None:
                img_feat = torch.cat((self.places_feat[example.img_key][...], img_feat), 0)
            # logger.info(f"img_feat.shape after: {img_feat.shape}")  # debug
    ...
    def tensorize_example(self, example, cls_token_at_end=False, pad_on_left=False,
                    ...
                    cls_token_segment_id=1, pad_token_segment_id=0,
                    mask_padding_with_zero=True):
        # logger.info("tensorize_example:")  # debug: used during the training
        tokens_a = self.tokenizer.tokenize(example.text_a)
        ...
        if example.text_b:
            ...
            if self.places_io_pred is not None:
                places_obj_tags += self.places_io_pred[example.img_key] + " "
            example_text_b = places_obj_tags + example.text_b

            txt_b_arr = example_text_b.split(';')
            txt_label_ixs = []
            ...
            txt_b = example_text_b.replace(';', ' ').strip()
        ...
        if self.args.img_feature_type.startswith('dis_code'):
            ...
            # logger.info(f"img_feat.shape before: {img_feat.shape}")  # debug
            if self.places_feat is not None:
                img_feat = torch.cat((torch.tensor(self.places_feat[example.img_key][...], dtype=torch.long), img_feat), 0)
            # logger.info(f"img_feat.shape after: {img_feat.shape}")   # debug
...
def train(args, train_dataset, eval_dataset, model, tokenizer):
    ...
    #for epoch in train_iterator:
        ...
        if args.wandb_entity != "":
            wandb.log({
                "Train Loss": total_loss / len(train_dataset),
                "Epoch": int(epoch + 1),
                "Global Step": global_step
            })

    if args.wandb_entity != "":
        wandb.run.summary["Best Eval Accuracy"] = 100*best_score
...
def evaluate(args, model, eval_dataset=None, prefix=""):
    ...
    label2ans = np.array([label2ans[l] for l in label2ans])
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        #for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                for out, lbl in zip(logits.argmax(1), batch[3]):
                    if lbl[0] > 1:
                        len_opn += 1
                        if out == lbl[0]:
                            correct_opn_num += 1
                ...
                for i in range(len(batch[3])):
                    s = softmax(logits.tolist()[i])
                    ind = np.argpartition(s, -5)[-5:]    # get top 5 predictions
                    ind = ind[np.argsort(s[ind])][::-1]  # sort them from best to worst

                    question = eval_dataset.tokenizer.convert_ids_to_tokens(batch[0][i].tolist())
                    question = " ".join(question[1:question.index("[SEP]")])

                    top5_predictions.append({
                        "type": "open" if batch[3][i][0] > 1 else "binary",
                        "question": question,
                        "true_answer": label2ans[batch[3][i].tolist()].tolist(),
                        "predicted": label2ans[ind].tolist(),                     # top 5 argmaxes
                        "scores": s[ind].tolist()                                 # top 5 scores
                    })
        ...
        acc_bin = float(correct_bin_num) / len_bin
        acc_opn = float(correct_opn_num) / len_opn

        logger.info("Eval Results:")
        logger.info("Eval Accuracy: %.3f" % (100*acc))
        logger.info("Eval Loss: %.3f" % (eval_loss))
        logger.info("Eval Binary: %.3f" % (100*acc_bin))
        logger.info("Eval Open: %.3f" % (100*acc_opn))
        if args.wandb_entity != "" and prefix != "":
            wandb.log({
                "Eval Accuracy": 100*acc,
                "Eval Loss": eval_loss,
                "Global Step": int(prefix),
                "Eval Binary": 100*acc_bin,
                "Eval Open": 100*acc_opn
            })
        top5_pred_output = f"top5-{prefix}" if prefix != "" else f"top5-finalOrBest"
        top5_pred_output_final = top5_pred_output
        i = 0
        while os.path.exists(os.path.join(eval_output_dir, f"{top5_pred_output_final}.json")):
            i += 1
            top5_pred_output_final = f"{top5_pred_output}-{i}"
        top5_pred_output_final = os.path.join(eval_output_dir, f"{top5_pred_output_final}.json")
        logger.info(f"Top 5 predictions saved to: {top5_pred_output_final}")
        json.dump(top5_predictions, open(top5_pred_output_final, "w"))
...
def main():
    ...
    parser.add_argument('--places_io_json', type=str, default='', help="Json file with places Indoor/Outdoor predictions.")
    parser.add_argument('--places_c_json', type=str, default='', help="Json file with places categories predictions.")
    parser.add_argument('--places_feats_hdf5', type=str, default='', help="H5 file with places features.")
    parser.add_argument('--wandb_entity', type=str, default='', help="Login for wandb reports.")
    ...
    # wandb
    if args.wandb_entity != "":
        # Get the run name... e.g., args.output_dir = results01-VinVL_test -> name = VinVL_test
        name = args.output_dir if re.match(r"^(result|output)", args.output_dir.lower()) is None else \
               re.match(r"^[0-9a-zA-Z]*[-_]?(.*)", args.output_dir).group(1)

        # Init log
        run = wandb.init(project="VinVL+L", entity=args.wandb_entity, name=name, config=vars(args))
        run.log({
            "Eval Accuracy": .0,
            "Eval Binary": 0,
            "Eval Open": 0,
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        best_model_dir = args.model_name_or_path
